...
def match_columns(csv_columns, model_columns, threshold=80):
    """Match CSV columns to the closest model feature names using fuzzy matching."""
    matched_columns = {}
    
    for col in csv_columns:
        best_match, score = process.extractOne(col, model_columns)  
        if score >= threshold:  
...
def detect_fraud(df):
    """Detect fraud transactions from the uploaded CSV file."""
    best_model_name = find_best_model(df)
    
    if not best_model_name:
        st.error("❌ No suitable model found for this CSV format.")
        return pd.DataFrame()  # Return an empty DataFrame for error handling

    st.success(f"✅ Using model: {best_model_name}")
    
    # Preprocess the CSV by matching columns and keeping only relevant features
    df = preprocess_csv(df, best_model_name)
    
    # Predict probabilities using the selected model
    selected_model = models[best_model_name]
    probabilities = selected_model.predict_proba(df)[:, 1]  # Fraud probability assumed at index 1
    df['Fraud_Probability'] = probabilities

    # Filter fraudulent transactions (Threshold = 0.7)
    fraud_df = df[df['Fraud_Probability'] > 0.7]
...
if menu == "CSV Upload":
    ...
    if uploaded_file is not None:
        ...
        if df.empty:
            ...
            # Detect fraud using enhanced logic with `match_columns()`
            fraud_df = detect_fraud(df)

            if not fraud_df.empty:
                st.subheader("🚨 Fraud Transactions Detected")
                st.dataframe(fraud_df)

                # Pie Chart Visualization
                fraud_counts = fraud_df['Fraud_Probability'].apply(lambda x: 'Fraud' if x > 0.7 else 'Non-Fraud').value_counts()
                fig, ax = plt.subplots(figsize=(6, 6))
                ax.pie(fraud_counts, labels=fraud_counts.index, autopct="%1.1f%%", colors=["red", "green"], startangle=140)
                st.pyplot(fig)
